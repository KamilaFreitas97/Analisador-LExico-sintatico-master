...
public class Analex {
	...
	public List<TokenLexema> analex() {
		...
		for (i = 0; i < cadeiaArray.length; i++) {
			...
			if (!this.ehPontoVirgula(cadeiaArray[i]))
                        if (!this.ehLiteralFlutuante(cadeiaArray[i]))
	...
	public boolean ehLiteralInteiro(char c) {
       ...
       public boolean ehLiteralFlutuante(char c) {
		if (Character.isDigit(c)) {
			String digito = "";
			do {
				digito += cadeiaArray[i] + "";
			...
			char cadeiaDigito[];
			cadeiaDigito = digito.toCharArray();
			int j = 0;
			int cont = 0;
			while(j < cadeiaDigito.length) {
				if(cadeiaDigito[j] == Token.LITERAL_FLUTUANTE.getValor())
					cont++;
				j++;
			}
			if(cont > 0) {
				this.tokenLexemaList.add(new TokenLexema(Token.LITERAL_FLUTUANTE, digito));
			}else {
				this.tokenLexemaList.add(new TokenLexema(Token.LITERAL_INTEIRO, digito));	
			}
